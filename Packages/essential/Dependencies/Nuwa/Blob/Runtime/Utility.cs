...
namespace Nuwa.Blob
{
    public static class Utility
    {
        ...
        public static BlobBuilderArray AllocateDynamicArray(this BlobBuilder builder, Type elementType, IntPtr arrayPtr, int length)
        {
            var func = typeof(Utility).GetMethod(nameof(AllocateDynamicArray), BindingFlags.Static | BindingFlags.NonPublic);
            return (BlobBuilderArray) func.MakeGenericMethod(elementType).Invoke(null, new object[] {builder, arrayPtr, length});
        }

        private static unsafe BlobBuilderArray AllocateDynamicArray<T>(BlobBuilder builder, IntPtr arrayPtr, int length) where T : unmanaged
        {
            ref var data = ref UnsafeUtility.AsRef<BlobArray<T>>(arrayPtr.ToPointer());
            var arrayBuilder = builder.Allocate(ref data, length);
            return new BlobBuilderArray(new IntPtr(arrayBuilder.GetUnsafePtr()), length, typeof(T));
        }
