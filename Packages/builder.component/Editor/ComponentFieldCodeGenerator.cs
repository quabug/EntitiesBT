...
namespace EntitiesBT.Editor
{
    ...
    public class BlobVariantFieldCodeGenerator : INodeDataFieldCodeGenerator
    {
        ...
        public string VariantPropertyNameSuffix = "Property";
        ...
        public string GenerateField(FieldInfo fi)
        {
            ...
            stringBuilder.AppendLine($"public {VariantInterfaceNamespace}.{variantType.Name}{VariantPropertyNameSuffix} {fi.Name};");
            ...
            void GenerateVariantInterface()
            {
                ...
                var filepath = $"{directory}/{variantType.Name}{VariantPropertyNameSuffix}.cs";
                if (!File.Exists(filepath) || File.ReadLines(filepath).FirstOrDefault() == _HEAD_LINE)
                {
                    using (var writer = new StreamWriter(filepath))
                    {
                        ...
                        writer.WriteLine(VariantGenerator.CreateInterface(variantType, VariantPropertyNameSuffix));
                        // writer.WriteLine(VariantGenerator.CreateClass(variableType, typeof(CustomVariantPropertyReader<>), VariantPropertyNameSuffix));
                        // writer.WriteLine(VariantGenerator.CreateClass(variableType, typeof(ComponentVariantPropertyReader<>), VariantPropertyNameSuffix));
                        writer.WriteLine(VariantGenerator.CreateClass(variantType, typeof(NodeVariantReader<>), VariantPropertyNameSuffix));
                        // writer.WriteLine(VariantGenerator.CreateClass(variableType, typeof(ScriptableObjectVariantPropertyReader<>), VariantPropertyNameSuffix));
